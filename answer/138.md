...
### 다른 풀이
```
다른 풀이 1(우에다) $ eval echo {$(< os.csv )},{$(< browser.csv)},{$(< service.csv)} | xargs -n 1 | tr , ' ' | awk '($2!="IE"||$1=="Windows")&&($2!="Safari"||$1=="macOS")'
다른 풀이 2（中村）$ cat os.csv | tr , \\n | xargs -I@ bash -c 'cat browser.csv | tr , \\n | xargs -I% echo @ %' | xargs -I@ bash -c 'cat service.csv | tr , \\n | xargs -I% echo @ %' | awk '($2!="IE"||$1=="Windows")&&($2!="Safari"||$1=="macOS")'
다른 풀이 3（田代）$ cat os.csv browser.csv service.csv | sed 's/[^,]*/@&@/g' | sed 's/^.*$/{&}/' | tr -d '\n' | sed 's/^/echo /' | bash | tr ' ' '\n' | sed 's/^.//;s/.$//' | tr @ ' ' | tr -s ' ' | awk '($2!="IE"||$1=="Windows")&&($2!="Safari"||$1=="macOS")'
다른 풀이 4（eban）$ join <(sed 's/^\|,/\no /g' os.csv) <(sed 's/^\|,/\no /g' browser.csv) | join - <(sed 's/^\|,/\no /g' service.csv) | grep -v '[^s] IE\|[^S] Safari\|^$'
다른 풀이 5（eban）$ parallel echo {1} {2} {3} ::: $(tr , '\n' <os.csv) ::: $(tr , '\n' <browser.csv) ::: $(tr , '\n' <service.csv) | grep -v '[^s] IE\|[^S] Safari'
다른 풀이 6(야마다) $ pict <(grep -H . {os,browser,service}.csv | sed '$aIF[browser.csv] = "IE" THEN[os.csv] = "Windows"; IF[browser.csv] = "Safari" THEN[os.csv] = "macOS";') /o:3
```
